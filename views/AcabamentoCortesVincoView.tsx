...
const FacaDefectCounter: React.FC<{
  ...
  const confirmAllModal = () => {
    ...
    Array.from({ length: numFacas }, (_, i) => i + 1).forEach(f => onUpdate(f, count));
    setAllModal(null);
  };
  ...
  return (
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 p-3">
      <div className="flex items-center gap-2 mb-2.5">
        ...
        <button type="button" onClick={() => setAllModal({ value: '' })}
        ...
        {total > 0 && (
          <span className="text-[10px] font-black text-white bg-rose-500 rounded-full min-w-[20px] h-5 flex items-center justify-center px-1.5">{total}</span>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {Array.from({ length: numFacas }, (_, i) => i + 1).map(faca => {
          ...
          return (
            <button key={faca} type="button" onClick={() => setModal({ faca, value: String(facaCounts[faca] ?? 0) })}
              className={`relative flex flex-col items-center justify-center w-10 h-10 rounded-xl border-2 transition-all ${
                active
              ...
              <span className="material-symbols-outlined text-[12px] leading-none">content_cut</span>
              <span className="text-[8px] font-black leading-none mt-0.5">{faca}</span>
              {active && (
                <span className="absolute -top-1.5 -right-1.5 min-w-[14px] h-3.5 rounded-full bg-rose-500 text-white text-[8px] font-black flex items-center justify-center px-0.5">
                  {count}
                </span>
              )}
            </button>
      ...
      {onDescricaoChange !== undefined && (
        <div className="mt-2.5 pt-2.5 border-t border-slate-200 dark:border-slate-700">
          ...
          <textarea value={descricao ?? ''} onChange={e => onDescricaoChange(e.target.value)}
            placeholder="Ex: rebarbas, faca torta..." rows={2}
      ...
      {modal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/60 backdrop-blur-sm p-4" onClick={() => setModal(null)}>
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-2xl w-72" onClick={e => e.stopPropagation()}>
            ...
            <input type="number" min={0} value={modal.value} autoFocus
            ...
            <div className="mt-3 flex gap-2">
              <button type="button" onClick={() => setModal(null)}
                className="flex-1 h-10 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-black text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800">Cancelar</button>
              <button type="button" onClick={confirmModal}
                className="flex-1 h-10 rounded-xl bg-indigo-600 text-white text-xs font-black hover:bg-indigo-700">Confirmar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {allModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/60 backdrop-blur-sm p-4" onClick={() => setAllModal(null)}>
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-2xl w-72" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              <div className="size-9 rounded-xl bg-indigo-100 dark:bg-indigo-950/30 flex items-center justify-center">
                <span className="material-symbols-outlined text-indigo-600 text-base">content_cut</span>
              ...
              <div>
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Todas as Posições</p>
                <p className="text-sm font-black text-slate-800 dark:text-white">{name}</p>
              </div>
            </div>
            <label className="text-[9px] font-black uppercase tracking-widest text-slate-400">Quantidade por posição</label>
            <input type="number" min={0} value={allModal.value} autoFocus
              onChange={e => setAllModal(prev => prev ? { ...prev, value: e.target.value } : prev)}
              onKeyDown={e => e.key === 'Enter' && confirmAllModal()}
              className="mt-1.5 h-12 w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-3 text-xl font-black outline-none focus:ring-2 focus:ring-indigo-500/20 text-center" />
            <p className="text-[9px] text-slate-400 mt-1.5 text-center">Será aplicado às {numFacas} posições</p>
            <div className="mt-3 flex gap-2">
              <button type="button" onClick={() => setAllModal(null)}
                className="flex-1 h-10 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-black text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800">Cancelar</button>
              <button type="button" onClick={confirmAllModal}
                className="flex-1 h-10 rounded-xl bg-indigo-600 text-white text-xs font-black hover:bg-indigo-700">Confirmar</button>
            </div>
          </div>
        </div>
  ...
  <div className={`flex flex-col gap-1 p-3 rounded-xl border ${colorClass}`}>
    ...
    <div className="flex items-center gap-1.5 mt-1">
      <button type="button" onClick={() => onChange(Math.max(0, value - 1))}
        className="size-7 rounded-lg bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 flex items-center justify-center text-slate-500 text-sm font-bold">−</button>
      <input type="number" value={value} onChange={e => onChange(Math.max(0, Number(e.target.value) || 0))}
        className="flex-1 h-7 bg-transparent text-center font-black text-sm outline-none min-w-0" />
...
const AcabamentoCortesVincoView: React.FC = () => {
  ...
  useEffect(() => {
    if (!op.trim()) { setOpFound(null); setSaldoInspecao(null); return; }
    const opUpper = op.trim().toUpperCase();

    supabase.from('orders').select('unidades_por_folha').eq('op', opUpper).maybeSingle()
      .then(({ data }) => {
        if (data) {
          const fromOp = data.unidades_por_folha ?? 0;
          if (fromOp > 0) setManualFacas(fromOp);
          setOpFound(true);
        } else { setOpFound(false); }
      });

    Promise.all([
      supabase.from('inspections').select('observations').eq('op', opUpper)
        .order('created_at', { ascending: false }).limit(20),
      supabase.from('acabamento_registros').select('qty_revisadas')
        .eq('op', opUpper).eq('modulo', 'corte_vinco'),
    ]).then(([inspRes, cvRes]) => {
  ...
  useEffect(() => { setNumFacas(manualFacas); }, [manualFacas]);

  const loadRecent = useCallback(async () => {
    setLoadingRecent(true);
    const { data } = await supabase.from('acabamento_registros')
      .select('id, op, qty_revisadas, qty_aprovadas, qty_reprovadas, timestamp, defects')
      .eq('modulo', 'corte_vinco').order('timestamp', { ascending: false }).limit(8);
  ...
  const handleUpdate = (defectKey: string, faca: number, count: number) => {
    setFacaCounts(prev => ({ ...prev, [defectKey]: { ...prev[defectKey], [faca]: count } }));
  };

  const totalDefects = totalAllDefects(facaCounts);

  const handleClear = () => {
    setOp(''); setSelectedOperatorIds([]); setSelectedMachineId('');
    setQtyRevisadas(0); setQtyEscolha(0); setFacaCounts(emptyFacaCounts());
    setOutrosDescricao(''); setNotes(''); setNumFacas(0); setManualFacas(0);
    setOpFound(null); setSaldoInspecao(null);
  };
  ...
  const handleSave = async () => {
    ...
    if (selectedOperatorIds.length === 0)   { showToast('Selecione ao menos um operador antes de registrar', 'error'); return; }
    if (qtyRevisadas === 0)                 { showToast('Informe a quantidade revisada', 'error'); return; }
    ...
    for (const d of DEFECTS) {
      ...
      if (entries.length > 0) facaDefectsPayload[d.key] = Object.fromEntries(entries) as Record<string, number>;
    }
    if (outrosDescricao.trim()) facaDefectsPayload['outros_descricao'] = outrosDescricao.trim();
    ...
    if (error) { console.error('Erro ao salvar:', error); showToast('Erro ao salvar registro', 'error'); return; }
  ...
  const aprovadas = Math.max(0, qtyRevisadas - qtyEscolha);
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <div className="flex-1 p-4 md:p-6 max-w-6xl mx-auto w-full">
        ...
        <div className="mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Corte e Vinco</h1>
              <p className="text-xs text-slate-500">Registro de defeitos por posição de faca</p>
            </div>
          </div>
          {profile?.name && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900/40 self-start">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-5 items-start">
          ...
          {/* ══ COLUNA ESQUERDA: Identificação + Quantidades + Obs + Histórico ══ */}
          <div className="space-y-4 lg:sticky lg:top-4">
            ...
            <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
              <h2 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 flex items-center gap-1.5">
                <span className="material-symbols-outlined text-sm text-indigo-400">badge</span>Identificação
              </h2>

              {/* OP */}
              <div className="mb-3">
                <label className="text-[9px] font-black uppercase tracking-widest text-slate-400">Número da OP</label>
                <input list="cv-op-list" value={op} onChange={e => setOp(e.target.value)} placeholder="Ex: 12345"
                  className="mt-1.5 h-10 w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-3 text-sm font-bold outline-none focus:ring-2 focus:ring-indigo-500/20" />
                <datalist id="cv-op-list">{opList.map(o => <option key={o} value={o} />)}</datalist>
                {opFound === false && <p className="text-[10px] text-rose-500 mt-1">OP não encontrada no cadastro</p>}
                {opFound === true  && <p className="text-[10px] text-emerald-600 mt-1 flex items-center gap-1"><span className="material-symbols-outlined text-xs">check_circle</span>OP encontrada</p>}
              </div>
              ...
              <div className="mb-3">
                ...
                <select value={selectedMachineId} onChange={e => setSelectedMachineId(e.target.value)}
                  className={`mt-1.5 h-10 w-full rounded-xl border px-3 text-sm font-bold outline-none focus:ring-2 focus:ring-indigo-500/20 bg-slate-50 dark:bg-slate-800 ${
                    !selectedMachineId ? 'border-rose-300 dark:border-rose-700 text-slate-400' : 'border-slate-200 dark:border-slate-700'
                  }`}>
                  <option value="">Selecione a máquina</option>
                  {machines.map(m => <option key={m.id} value={m.id}>{m.name}{m.code ? ` (${m.code})` : ''}</option>)}
                </select>
                {machines.length === 0 && <p className="text-[10px] text-amber-500 mt-1">Nenhuma máquina cadastrada para Corte e Vinco.</p>}
              </div>
              ...
              <div className="mb-3">
                <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1 mb-1.5">
                  ...
                  <span className="ml-auto text-[8px] font-bold text-slate-400 normal-case tracking-normal">Todos que trabalharam</span>
                </label>
                {operators.length === 0 ? (
                  <p className="text-[10px] text-amber-500">Nenhum operador cadastrado para Corte e Vinco.</p>
                ) : (
                  <div className="flex flex-wrap gap-1.5">
                    {operators.map(o => {
                      ...
                      return (
                        <button key={o.id} type="button"
                          onClick={() => setSelectedOperatorIds(prev => selected ? prev.filter(id => id !== o.id) : [...prev, o.id])}
                          className={`px-2.5 py-1.5 rounded-lg border text-xs font-bold transition-colors ${
                            selected ? 'bg-indigo-600 border-indigo-600 text-white' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/20'
                          }`}>
                ...
                {selectedOperatorIds.length === 0 && operators.length > 0 && (
                  <p className="text-[10px] text-rose-500 mt-1 flex items-center gap-1">
                    <span className="material-symbols-outlined text-xs">warning</span>Selecione ao menos um operador
                  </p>
                )}
              </div>
              ...
              {/* Facas */}
              <div>
                <label className="text-[9px] font-black uppercase tracking-widest text-slate-400">Quantidade de Facas</label>
                <div className="mt-1.5 flex items-center gap-3">
                  <input type="number" min={1} max={64} value={manualFacas || ''} placeholder="Ex: 8"
                    onChange={e => setManualFacas(Math.max(0, Number(e.target.value) || 0))}
                    className="w-24 h-10 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-3 text-sm font-black outline-none focus:ring-2 focus:ring-indigo-500/20" />
                  {opFound === true && numFacas > 0 && (
                    <span className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 flex items-center gap-1">
                      <span className="material-symbols-outlined text-sm">check_circle</span>{numFacas} da OP
                    </span>
                  )}
                </div>
              </div>
            ...
            <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
              <h2 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 flex items-center gap-1.5">
                <span className="material-symbols-outlined text-sm text-indigo-400">numbers</span>Quantidades
              </h2>
              <div className="grid grid-cols-2 gap-3">
                <QtyCard label="Rodadas" value={qtyRevisadas} onChange={setQtyRevisadas}
                  colorClass="border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50" />
                <QtyCard label="Escolha → Rev. Final" value={qtyEscolha} onChange={setQtyEscolha}
                  colorClass="border-amber-100 dark:border-amber-900/30 bg-amber-50 dark:bg-amber-950/20" />
              </div>
              {qtyRevisadas > 0 && (
                <div className="mt-3 flex items-center justify-between px-3 py-2 rounded-xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900">
                  <span className="text-[9px] font-black uppercase tracking-widest text-emerald-600">Aprovadas direto</span>
                  <span className="text-sm font-black text-emerald-700 dark:text-emerald-300">{fmt(aprovadas)} un.</span>
                </div>
              )}
            </section>
            ...
            <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
              <h2 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 flex items-center gap-1.5">
                <span className="material-symbols-outlined text-sm text-slate-400">notes</span>Observações
              </h2>
              <textarea value={notes} onChange={e => setNotes(e.target.value)}
                placeholder="Anotações sobre o lote..." rows={3}
            ...
            <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
              <h2 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3 flex items-center gap-1.5">
                <span className="material-symbols-outlined text-sm text-slate-400">history</span>Registros Recentes
              </h2>
              {loadingRecent ? (
                ...
                <div className="flex flex-col gap-2">
                  {recentRecords.map(r => {
                    const totalD = Object.values(r.defects ?? {}).reduce<number>((s, v) => s + (typeof v === 'number' ? v : 0), 0);
                    return (
                      <div key={r.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/50">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {totalD > 0 && (
                              <span className="px-1.5 py-0.5 rounded-full bg-rose-100 dark:bg-rose-950/30 text-rose-600 text-[9px] font-black">{fmt(totalD)} def.</span>
                            )}
                          </div>
                          <p className="text-[10px] text-slate-400">{fmt(r.qty_revisadas)} rod. · {fmt(r.qty_reprovadas)} escolha</p>
                        </div>
          ...
          {/* ══ COLUNA DIREITA: Rastreio + Saldo + Defeitos ══════════════════ */}
          <div className="space-y-4">
            ...
            {/* Saldo recebido */}
            {saldoInspecao && (
              <div className="flex items-start gap-3 px-4 py-3.5 rounded-2xl bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-200 dark:border-indigo-800">
                <span className="material-symbols-outlined text-indigo-500 text-xl mt-0.5 shrink-0">assignment_turned_in</span>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-lg font-black text-indigo-800 dark:text-indigo-200 mt-0.5">
                    {fmt(saldoInspecao.em_escolha)} <span className="text-sm font-bold text-indigo-500">un. em escolha</span>
                  </p>
                  {saldoInspecao.rodadas > 0 && (
                    <p className="text-[10px] text-indigo-500">{fmt(saldoInspecao.rodadas)} unidades rodadas no total</p>
                  )}
                </div>
              </div>
            )}
            ...
            <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
              <div className="flex items-center justify-between mb-4">
                ...
                {totalDefects > 0 && (
                  <span className="px-2.5 py-1 rounded-full bg-rose-100 dark:bg-rose-950/30 text-rose-600 text-[10px] font-black">
              ...
              {numFacas === 0 ? (
                <div className="py-10 text-center">
                  <span className="material-symbols-outlined text-5xl text-slate-200 dark:text-slate-700">content_cut</span>
                  <p className="text-sm font-bold text-slate-400 mt-3">Informe a quantidade de facas na identificação</p>
                  <p className="text-xs text-slate-300 dark:text-slate-600 mt-1">Ex: 8 facas para uma embalagem simples</p>
                </div>
              ) : (
      ...
      {/* ── Footer fixo ──────────────────────────────────────────────────────── */}
      <div className="sticky bottom-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 px-4 py-3 flex gap-3">
        <button type="button" onClick={handleClear}
          className="h-11 px-5 rounded-xl border border-slate-200 dark:border-slate-700 text-sm font-black text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
          Limpar
        </button>
        <button type="button" onClick={handleSave}
          disabled={saving || !op.trim() || !selectedMachineId || selectedOperatorIds.length === 0}
          className="flex-1 h-11 rounded-xl bg-indigo-600 text-white text-sm font-black hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
          {saving
